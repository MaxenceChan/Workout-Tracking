...
function Analytics({ sessions, sessionTemplates = [] }) {
  ...
  const [muscleSplitLoading, setMuscleSplitLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!sessions.length) {
        if (active) setMuscleSplitAI([]);
        return;
      }
      setMuscleSplitLoading(true);
      const split = await buildMuscleSplitByAI(sessions);
      if (active) {
        setMuscleSplitAI(split);
        setMuscleSplitLoading(false);
      }
    };
    run();
    return () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Catégorisation IA (HuggingFace) avec fallback RAG musculaire.
          </p>
        </div>
...
const formatMuscleLabel = (value) =>
  ...
  value
    ...
    .join(" ");
const MUSCLE_AI_CACHE_KEY = "workout-tracker-muscle-ai-cache-v1";
...
const readMuscleAiCache = () => {
  try {
    const raw = localStorage.getItem(MUSCLE_AI_CACHE_KEY);
    return raw ? JSON.parse(raw) : {};
  } catch (error) {
    console.error("Muscle AI cache read error", error);
    return {};
  }
};
const writeMuscleAiCache = (cache) => {
  try {
    localStorage.setItem(MUSCLE_AI_CACHE_KEY, JSON.stringify(cache));
  } catch (error) {
    console.error("Muscle AI cache write error", error);
  }
};
...
const fetchMusclesFromAI = async (exerciseName) => {
  const response = await fetch("/api/muscle-categorize", {
    method: "POST",
  ...
  return Array.isArray(payload?.muscles) ? payload.muscles : [];
};
const resolveExerciseMuscles = async (exerciseName, cache) => {
  if (!exerciseName) return [];
  const normalized = normalizeExerciseName(exerciseName);
  if (!normalized) return [];
  if (cache[normalized]) return cache[normalized];
  try {
    const muscles = await fetchMusclesFromAI(exerciseName);
    if (muscles.length > 0) {
      cache[normalized] = muscles;
      return muscles;
    }
  } catch (error) {
    console.error("AI muscle fetch failed", error);
  }
  const ragMuscles = getMusclesFromRag(exerciseName);
  cache[normalized] = ragMuscles;
  return ragMuscles;
};
async function buildMuscleSplitByAI(sessions) {
  const map = new Map();
  const cache = readMuscleAiCache();
  const exercises = sessions.flatMap((session) => session.exercises || []);
  const uniqueExercises = Array.from(
    new Set(exercises.map((exercise) => exercise.name).filter(Boolean))
  );
  const musclesByExercise = {};
  await Promise.all(
    uniqueExercises.map(async (exerciseName) => {
      musclesByExercise[exerciseName] = await resolveExerciseMuscles(
        exerciseName,
        cache
      );
    })
