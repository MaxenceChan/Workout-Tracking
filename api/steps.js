...
export default async function handler(req, res) {
  try {
    ...
    // Load stepsCache (single field, already fetched above — 1 Firestore read total)
    ...
    // First load (cache > 300 days old): fetch 12×30-day chunks in parallel = 360 days
    // Subsequent loads: only fetch latest 30 days (delta)
    const chunks =
    ...
    try {
      ...
      const invalidGrant =
        apiError === "invalid_grant" || msg.includes("invalid_grant") || status === 401;

      if (invalidGrant) {
        await userRef.set(
          {
            "googleFit.needs_reauth": true,
            "googleFit.last_error": { code: apiError || status || "invalid_grant", message: error?.message || "" },
      ...
      // Other Google Fit error — return stored cache so user keeps their data
      return res.status(200).json({ connected: true, needsReauth: false, steps: storedSteps, fromCache: true });
    }

    // Merge: fresh data overwrites cache, keep older cached dates untouched
    ...
    // Save to Firestore only if something changed
